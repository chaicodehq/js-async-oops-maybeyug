...
export function createSamosaCart(ownerName, location) {
  ...
  const cartFunction = {
    ...
    getDailySales() {
      ...
      const result = this.sales.reduce((total, curr) => {
        total += curr.total;
        return total;
      }, 0);
      return result;
    ...
    getPopularItem() {
      ...
      const result = [...this.sales].sort((a, b) =>
        b.quantity >= a.quantity ? 1 : -1,
      );
